 ...
 */
	...
	public:
		...
		MessageNode(self_type&& rhs) : base_type(std::move(rhs)), inputs(std::move(rhs.inputs)),
		...
		virtual ~MessageNode() { clear(); } //might want to lock while deleting links 
	...
	using stdMessageNode = MessageNode< InPort< Buffer<S,L> >, OutPort< Buffer<S,L> > >;
